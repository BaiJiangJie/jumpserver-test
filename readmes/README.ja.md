...
JumpServer は、DevOps および IT チームに対して、Web ブラウザを介して SSH、RDP、Kubernetes、データベース、リモートアプリケーションのエンドポイントへのオンデマンドでセキュアなアクセスを提供するオープンソースの特権アクセス管理 (PAM) ツールです。
...
JumpServer は複数の主要コンポーネントで構成されており、それらが集まって JumpServer の機能フレームワークを形成し、ユーザーに対して運用管理とセキュリティ制御のための包括的な機能を提供します。
...
貢献するための PR の提出を歓迎します。ガイドラインについては [CONTRIBUTING.md][contributing-link] を参照してください。

## セキュリティ

JumpServer はミッションクリティカルな製品です。インストールおよび展開のための基本的セキュリティ推奨に従ってください。セキュリティ関連の問題が発生した場合は、直接ご連絡ください：
